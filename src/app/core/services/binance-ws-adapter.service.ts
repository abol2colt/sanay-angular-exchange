...
export class BinanceWsAdapterService {
  ...
  connect(symbols: string[], handlers: BinanceWsHandlers): WebSocket | null {
    ...
    if (!socketUrl) {
      handlers.onError?.('binance_empty_symbols');
      return null;
    }
  ...
  private buildStreamNames(symbols: string[]): string[] {
    return [...new Set(symbols.map((symbol) => this.normalizeSymbol(symbol)).filter(Boolean))].map(
      (symbol) => `${symbol}${QUOTE_ASSET}@ticker`,
    );
  }
